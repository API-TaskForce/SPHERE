...
class OrganizationInvitationRepository extends RepositoryBase {
  async findById(id: string) {
    try {
      const inv = await OrganizationInvitationMongoose.findById(id);
  ...
  async findByOrganizationId(organizationId: string) {
    try {
      ...
      return invitations.map(inv =>
        inv.toObject({ getters: true, virtuals: true, versionKey: false })
      );
    } catch {
      return [];
    }
  ...
  async incrementUseCount(id: string) {
    return OrganizationInvitationMongoose.findOneAndUpdate(
      { _id: id },
      { $inc: { useCount: 1 } },
      { new: true }
    );
  ...
  async destroyByOrganizationId(organizationId: string) {
    const result = await OrganizationInvitationMongoose.deleteMany({
      _organizationId: new mongoose.Types.ObjectId(organizationId),
    });
