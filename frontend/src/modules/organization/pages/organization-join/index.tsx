...
import { Organization, OrganizationInvitation, useOrganizationsApi } from '../../api/organizationsApi';
...
export default function OrganizationJoinPage() {
  ...
  useEffect(() => {
    ...
    if (!authUser.isAuthenticated) {
      router.push(`/login?redirect=/organizations/join/${code}`);
      return;
    }
    if (!code) {
    ...
    previewInvitation(code)
      .then(({ invitation: inv, organization }) => {
        setInvitation(inv);
        setOrg(organization);
      })
  ...
  return (
    <div className="mx-auto max-w-md px-4 py-16">
      <div className="flex flex-col items-center gap-4 text-center">
        ...
        {org.description && (
          <p className="text-sm text-sphere-grey-600">{org.description}</p>
        )}
